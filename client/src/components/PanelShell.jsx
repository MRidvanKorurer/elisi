...
export function SectionTitle({ overline, title, subtitle, action }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap', mb: 2.4 }}>
      ...
      {action ? (
        <Box sx={{ flexShrink: 0, width: { xs: '100%', sm: 'auto' }, maxWidth: '100%' }}>
          {action}
        </Box>
      ) : null}
    </Box>
...
export default function PanelShell({
  ...
  const sidebar = (
    ...
    >
      <Box sx={{ px: 2.6, py: 3, display: 'flex', alignItems: 'center', gap: 1.3, flexShrink: 0 }}>
        <Box
          sx={{
            width: 42,
            height: 42,
      ...
      <Box sx={{ p: 2, m: 1.6, mt: 0.5, borderRadius: '18px', bgcolor: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.09)', flexShrink: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, mb: 1.2 }}>
          <Avatar sx={{ width: 36, height: 36, bgcolor: T.rose, fontWeight: 900, flexShrink: 0 }}>{initial}</Avatar>
          <Box sx={{ minWidth: 0 }}>
