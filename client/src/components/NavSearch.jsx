...
import { CATEGORY_ICON_NAMES, categoryLabel, normalize } from '../utils/categories';
...
const CategoryIcon = ({ value, iconName, ...props }) => (
  <DynamicIcon
    iconName={iconName || CATEGORY_ICON_NAMES[String(value || '').toLowerCase()] || 'CategoryOutlined'}
    {...props}
  />
);

/** Navbar araması: aktif kategoriler /categories API’sinden */
let categoriesPromise = null;
const loadCategories = () => {
  if (!categoriesPromise) {
    categoriesPromise = categoryService
      .getAllCategories()
      .then((data) => {
        const list = Array.isArray(data?.categories) ? data.categories : [];
        return list
          .map((item) => {
            const value = String(item.categoryId || item.id || item.slug || item.value || '')
              .toLowerCase()
              .trim();
            if (!value) return null;
            return {
              value,
              label: item.name || categoryLabel(value),
              iconName: item.iconName || CATEGORY_ICON_NAMES[value] || 'CategoryOutlined',
              order: Number(item.order) || 0,
              productCount: Number(item.productCount) || 0
            };
          })
          .filter(Boolean)
          .sort((a, b) => a.order - b.order || String(a.label).localeCompare(String(b.label), 'tr'));
      })
      .catch(() => []);
  }
...
export default function NavSearch({ solid = true, variant = 'desktop', onNavigate }) {
  ...
  const matchedCategories = useMemo(() => {
    ...
    return categories
      .filter((item) => {
        const hay = `${item.label} ${categoryLabel(item.value)} ${categoryLabel(item.value, t)} ${item.value}`;
  ...
  const options = useMemo(() => {
    ...
    if (!term) {
      ...
      categories
        ...
        .forEach((item) => list.push({
          kind: 'category',
          section: t('search.browseCategory', { ns: 'catalog' }),
          value: item.value,
          label: item.label,
    ...
    matchedCategories.forEach((item) => list.push({
      kind: 'category',
      section: t('search.categories', { ns: 'catalog' }),
      value: item.value,
      label: item.label,
      iconName: item.iconName,
      productCount: item.productCount
    ...
    return list;
  }, [term, recent, categories, matchedCategories, results, t]);
          ...
          <React.Fragment key={`${option.kind}-${option.value || option.product?._id || index}`}>
            ...
            {option.kind === 'category' && (
              ...
              >
                ...
                >
                  <CategoryIcon value={option.value} iconName={option.iconName} sx={{ fontSize: 19 }} />
                </Box>
                <Box sx={{ minWidth: 0, flex: 1 }}>
                  <Typography noWrap sx={{ fontSize: '0.88rem', fontWeight: 800, color: '#2E3B55' }}>
                    <Highlight text={option.label || categoryLabel(option.value, t)} term={term} />
                  </Typography>
                  <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: '#6E5252' }}>
                    {option.productCount > 0
                      ? t('search.categoryCount', { ns: 'catalog', count: option.productCount, defaultValue: `${option.productCount} ürün` })
                      : t('search.browseCollection', { ns: 'catalog', defaultValue: 'Koleksiyonu görüntüle' })}
                  </Typography>
                </Box>
