...
export default function AteliersPage() {
  ...
  useEffect(() => {
    setLimit((prev) => nextVisibleCount(prev, pageSize));
  }, [pageSize, queryKey]);

  useEffect(() => {
    if (limit < 1) return undefined;
    let cancelled = false;
    const refreshing = items.length > 0;
    ...
    atelierWeekService.directory({
      ...
      .then((data) => {
        if (cancelled) return;
        setItems(data?.ateliers || []);
        setFacets(data?.facets || { cities: [], crafts: [] });
        setPagination(data?.pagination || { page: 1, hasMore: false, total: 0 });
      })
      .catch(() => {
        if (!cancelled) {
          setItems([]);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- items.length only chooses spinner mode
  }, [queryKey, q, sehir, tur, sort, limit, t]);
      ...
      <SiteContainer>
        ...
        {error && !initialBusy ? (
          <Box sx={{ py: 8, textAlign: 'center' }}>
            ...
            <LoadingButton tone="outline" onClick={() => setLimit((prev) => Math.max(prev, pageSize) || pageSize)}>
